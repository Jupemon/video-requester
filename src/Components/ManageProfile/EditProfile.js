...
class VideoPrice extends Component {
  ...
  render() { 
    ...
    <Popover.Title as="h3">Set video price</Popover.Title>
        ...
        <Button onClick={() => {this.setState({price : this.state.input})}}>Save</Button>
        </Popover.Content>
      </Popover>}>
...
class Currency extends Component {
  ...
  render() { 
    ...
    return <Dropdown.Item key={cur} onClick={() => {this.setState({currentlySelected : cur})}}>{cur}</Dropdown.Item>
    })}
  </DropdownButton>
  );
  }
...
class EditProfile extends Component {
    ...
    render() { 
        return ( <div className="top-info">
              <Jumbotron>
              <h1><Username userName="JP. morgan"/></h1>
  <Row>
  <Col md="6">
  ...
  <Col>
  <p>Current video price is <VideoPrice price="50" currency="eur"/> <Currency /></p>
